...
def test_validate_route_workspace() -> None:
    ...
    response = client.post(
        "/validate",
        files={"json_file": ("workspace.json", BytesIO(payload), "application/json")},
    )
    assert response.status_code == 200
    assert "Validation results" in response.text


def test_validate_route_bad_json() -> None:
    client = TestClient(app)
    response = client.post(
        "/validate",
        files={"json_file": ("bad.json", BytesIO(b"{bad"), "application/json")},
    )
